    ...
    public static void init() {
        try {
            ...
            EntityAlienVillager.ITradeList[] newTradeList = new EntityAlienVillager.ITradeList[]{
                    ...
                    new EntityAlienVillager.ItemAndEmeraldToItem(new ItemStack(GAGItems.PLAN_TIER2, 1), new EntityAlienVillager.PriceInfo(3, 5), new ItemStack(GAGItems.PLAN_BUGGY_MOON)),
                    new EntityAlienVillager.ItemAndEmeraldToItem(new ItemStack(GAGItems.PLAN_BUGGY_MOON), new EntityAlienVillager.PriceInfo(3, 5), new ItemStack(GAGItems.PLAN_TIER2, 1)),
    ...
    private static void logTrades(Field tradeListField) throws IllegalAccessException {
        ...
        if (currentTrades != null && currentTrades.length > 0) {
            for (int i = 0; i < currentTrades.length; i++) {
                GagTweaks.LOGGER.info("Trade #" + i + ": " + currentTrades[i].getClass().getSimpleName() + " => " + currentTrades[i]);
            }
        } else {
            GagTweaks.LOGGER.info("DEFAULT_TRADE_LIST_MAP is already empty.");
        }
    }
}
