...
public class MEStockingBus extends MEInputBus {

    private static final int CONFIG_SIZE = 16;
    private boolean autoPull;
    private Predicate<ItemStack> autoPullTest;

    public MEStockingBus(ResourceLocation metaTileEntityId) {
    ...
    public MetaTileEntity createMetaTileEntity(IGregTechTileEntity iGregTechTileEntity) {
        return new MEStockingBus(metaTileEntityId);
    }

    @Override
    protected MEStockingBus.ExportOnlyAEStockingItemList getAEItemHandler() {
        if (this.aeItemHandler == null) {
            this.aeItemHandler = new MEStockingBus.ExportOnlyAEStockingItemList(this, CONFIG_SIZE, getController());
        }
        return (MEStockingBus.ExportOnlyAEStockingItemList) this.aeItemHandler;
    }
    ...
    protected void syncME() {
        ...
        for (MEStockingBus.ExportOnlyAEStockingItemSlot slot : this.getAEItemHandler().getInventory()) {
    ...
    private boolean testConfiguredInOtherBus(ItemStack stack) {
        ...
        if (!(controller instanceof RecipeMapMultiblockController rmmc) || !rmmc.isDistinct()) {
            ...
            for (var ability : abilityList) {
                if (ability instanceof MEStockingBus.ExportOnlyAEStockingItemList aeList) {
    ...
    private static class ExportOnlyAEStockingItemSlot extends ExportOnlyAEItemSlot {
        ...
        private final MEStockingBus holder;

        public ExportOnlyAEStockingItemSlot(IAEItemStack config, IAEItemStack stock,
                                            MEStockingBus holder) {
            super(config, stock);
            this.holder = holder;
        }

        public ExportOnlyAEStockingItemSlot(MEStockingBus holder) {
            super();
            this.holder = holder;
        }

        @Override
        public MEStockingBus.ExportOnlyAEStockingItemSlot copy() {
            return new MEStockingBus.ExportOnlyAEStockingItemSlot(
    ...
    private static class ExportOnlyAEStockingItemList extends ExportOnlyAEItemList {
        ...
        public ExportOnlyAEStockingItemList(MEStockingBus holder, int slots,
        ...
        protected void createInventory(MetaTileEntity holder) {
            if (!(holder instanceof MEStockingBus stocking)) {
                throw new IllegalArgumentException("Cannot create Stocking Item List for nonstocking MetaTileEntity!");
            }
            this.inventory = new MEStockingBus.ExportOnlyAEStockingItemSlot[size];
            for (int i = 0; i < size; i++) {
                this.inventory[i] = new MEStockingBus.ExportOnlyAEStockingItemSlot(stocking);
            }
        ...
        @Override
        public MEStockingBus.ExportOnlyAEStockingItemSlot[] getInventory() {
            return (MEStockingBus.ExportOnlyAEStockingItemSlot[]) super.getInventory();
        }
