...
public class MEStockingHatch extends MEInputHatch {

    private static final int CONFIG_SIZE = 16;
    private boolean autoPull;
    private Predicate<FluidStack> autoPullTest;

    public MEStockingHatch(ResourceLocation metaTileEntityId) {
    ...
    public MetaTileEntity createMetaTileEntity(IGregTechTileEntity iGregTechTileEntity) {
        return new MEStockingHatch(metaTileEntityId);
    }

    @Override
    protected MEStockingHatch.ExportOnlyAEStockingFluidList getAEFluidHandler() {
        if (this.aeFluidHandler == null) {
            this.aeFluidHandler = new MEStockingHatch.ExportOnlyAEStockingFluidList(this, CONFIG_SIZE, getController());
        }
        return (MEStockingHatch.ExportOnlyAEStockingFluidList) this.aeFluidHandler;
    }
    ...
    protected void syncME() {
        ...
        for (MEStockingHatch.ExportOnlyAEStockingFluidSlot slot : this.getAEFluidHandler().getInventory()) {
    ...
    private boolean testConfiguredInOtherHatch(FluidStack stack) {
        ...
        for (var ability : abilityList) {
            if (ability instanceof MEStockingHatch.ExportOnlyAEStockingFluidSlot aeSlot) {
    ...
    private static class ExportOnlyAEStockingFluidSlot extends ExportOnlyAEFluidSlot {
        ...
        public ExportOnlyAEStockingFluidSlot(MEStockingHatch holder, IAEFluidStack config,
                                             IAEFluidStack stock, MetaTileEntity entityToNotify) {
            super(holder, config, stock, entityToNotify);
        }

        public ExportOnlyAEStockingFluidSlot(MEStockingHatch holder, MetaTileEntity entityToNotify) {
            super(holder, entityToNotify);
        }

        @Override
        protected MEStockingHatch getHolder() {
            return (MEStockingHatch) super.getHolder();
        }

        @Override
        public ExportOnlyAEFluidSlot copy() {
            return new MEStockingHatch.ExportOnlyAEStockingFluidSlot(
                    this.getHolder(),
    ...
    private static class ExportOnlyAEStockingFluidList extends ExportOnlyAEFluidList {
        ...
        private final MEStockingHatch holder;

        public ExportOnlyAEStockingFluidList(MEStockingHatch holder, int slots,
        ...
        protected void createInventory(MetaTileEntity holder, MetaTileEntity entityToNotify) {
            if (!(holder instanceof MEStockingHatch stocking)) {
                throw new IllegalArgumentException("Cannot create Stocking Fluid List for nonstocking MetaTileEntity!");
            }
            this.inventory = new MEStockingHatch.ExportOnlyAEStockingFluidSlot[size];
            for (int i = 0; i < size; i++) {
                this.inventory[i] = new MEStockingHatch.ExportOnlyAEStockingFluidSlot(stocking, entityToNotify);
            }
        }

        @Override
        public MEStockingHatch.ExportOnlyAEStockingFluidSlot[] getInventory() {
            return (MEStockingHatch.ExportOnlyAEStockingFluidSlot[]) super.getInventory();
        }
